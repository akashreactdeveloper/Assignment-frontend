export const createTask = async (title, description, dueDate, priority, token) => {
  try {
    const response = await fetch('https://assignment-backend-urtz.onrender.com/api/tasks', {
      method: 'POST',
      headers: {
...
export const deleteTask = async (id, token) => {
  try {
    const response = await fetch(`https://assignment-backend-urtz.onrender.com/api/tasks/${id}`, {
      method: 'DELETE',
      headers: {
        'Authorization': `Bearer ${token}`,
      },
